...
def preprocess_data_view(df):
    ...
    numeric_cols = df.select_dtypes(include=['number']).columns
    scaler = joblib.load(r'scaler_final.pkl')
    df[numeric_cols] = scaler.transform(df[numeric_cols])
    df.update(df[numeric_cols])
...
# Initialize session state variables
    ...
    with col1:
        ...
        if st.session_state.status == "Fetching Customer Master Data...":
            ...
            st.session_state.customer_master.to_csv(r"C:\Users\Deloite\Desktop\cl_optimization_dashboard_results\customer_master.csv", index = False)
            # st.write(st.session_state.customer_master.shape)
    ...
    with col2:
        ...
        if st.session_state.status == "Fetching Sales Data...":
            ...
            sales_df.to_csv(r"C:\Users\Deloite\Desktop\cl_optimization_dashboard_results\sales_df.csv", index = False)
            progress_bar.progress(100)
            time.sleep(0.5)
            progress_bar.empty()
            st.rerun()
    ...
    with col3:
        ...
        if st.session_state.status == "Fetching Collections Data...":
            ...
            collections_df.to_csv(r"C:\Users\Deloite\Desktop\cl_optimization_dashboard_results\collections_df.csv", index = False)
            progress_bar.progress(100)
            time.sleep(0.5)
            progress_bar.empty()
            st.rerun()
    ...
    with col1:
        if st.button("🚀 Generate Transactions", disabled=not st.session_state.final_ready):
            ...
            st.session_state.transactions.to_csv(r"C:\Users\Deloite\Desktop\cl_optimization_dashboard_results\transactions.csv")
    ...
    with col2:
        ...
        if st.button("📌 Generate Knockoffs", disabled=(
            ...
            st.session_state.knock_offs.to_csv(r"C:\Users\Deloite\Desktop\cl_optimization_dashboard_results\knockoffs.csv")
    ...
    with col3:
        ...
        if st.button("💹 Generate Profitability", disabled=(
            ...
            st.session_state.profit_df.to_csv(r"C:\Users\Deloite\Desktop\cl_optimization_dashboard_results\profitability.csv")
    ...
    #         st.session_state.cl_std.to_csv(r"C:\Users\Deloite\Desktop\cl_optimization_dashboard_results\cl_std.csv")
    ...
    with col4:
        if st.button("📈 Generate Key Metrics", disabled=  not st.session_state.final_ready or 
            ...
            st.session_state.key_metrics.to_csv(r"C:\Users\Deloite\Desktop\cl_optimization_dashboard_results\key_metrics.csv")
    ...
    with col5:
        if st.button("✨ Generate Features File", disabled= not st.session_state.final_ready or 
            ...
            st.session_state.features.to_csv(r"C:\Users\Deloite\Desktop\cl_optimization_dashboard_results\features.csv")
...
if page == "Predictive Model for Customers":
    ...
    # if 'predictions' not in st.session_state:
    #     st.session_state.predictions = None
    path = r'C:\Users\Deloite\Desktop\Agcare-GIT'
    model_file = r'Random_forest_model_with20_features_SCL_20K.pkl'
    if model_file is not None:
        model = joblib.load(model_file)
        if st.session_state.features is not None:
            user_input_df = st.session_state.features
...
if page == "Customer Details":
    ...
    if st.session_state.predictions is not None or st.session_state.features is not None:
        ...
        if st.session_state.test_predictions is not None:
            Customer_level_data = pd.read_pickle(r'Data\customer_details_demo_data\key_metrics_combined_2547_v3.pkl')
            states = Customer_level_data['State'].unique()
            STATE = st.selectbox("Select State", options = states)
            trans_df = pd.read_pickle(r'Data\customer_details_demo_data\trans_df_combined_2547.pkl')
            profit_df = pd.read_pickle(r'Data\customer_details_demo_data\profit_df_combined_2547.pkl')
            features_df = pd.read_csv(r'Data\customer_details_demo_data\test_data_20_features.csv')
            current_credit_limits = pd.read_csv(r'Data\customer_details_demo_data\current_credit_limits_2547.csv')
            ...
            knock_df = pd.read_pickle(r'AgCare\app_phase_2_new\Data\customer_details_demo_data\knock_df_combined_2547.pkl')
